...
enum hand_enum { LEFT_HAND, RIGHT_HAND }
enum vr_mode_override_enum { VR_MODE_USE_CONFIG, VR_MODE_USE_FLAT, VR_MODE_USE_VR }
var vr_mode_override: int = vr_mode_override_enum.VR_MODE_USE_CONFIG

export (bool) var vr_mode_enabled: bool = true

enum vr_hmd_mirroring_enum { HMD_MIRROR_FLAT_UI, HMD_MIRROR_VR }
export (int) var vr_hmd_mirroring: int = vr_hmd_mirroring_enum.HMD_MIRROR_FLAT_UI

enum vr_control_type_enum { CONTROL_TYPE_CLASSIC, CONTROL_TYPE_DUAL_HAND_CONTROLLERS }
export (int) var vr_control_type: int = vr_control_type_enum.CONTROL_TYPE_DUAL_HAND_CONTROLLERS

enum body_awareness_enum {
	BODY_AWARENESS_HANDS_ONLY,
	BODY_AWARENESS_CONTROLLERS_ONLY,
	BODY_AWARENESS_FULL_BODY
}
export (int) var body_awareness: int = body_awareness_enum.BODY_AWARENESS_HANDS_ONLY

export (bool) var fov_comfort_mode: bool = true

export (bool) var movement_on_rotation_controller: bool = false
export (bool) var click_to_move: bool = false

export (bool) var strafe_movement: bool = true
export (bool) var always_move_at_full_speed: bool = true

export (float) var movement_deadzone: float = 0.2

export (float) var rotation_sensitivity: float = 1.0
export (float) var rotation_deadzone: float = 0.2

export (float) var max_positional_distance_start: float = 1.0
export (float) var max_positional_distance_end: float = 1.1

enum turning_mode_enum {
	TURNING_MODE_SMOOTH,
	TURNING_MODE_SNAP_30,
	TURNING_MODE_SNAP_45,
	TURNING_MODE_SNAP_90,
	TURNING_MODE_SNAP_CUSTOM
}
export (int) var turning_mode: int = turning_mode_enum.TURNING_MODE_SNAP_45
export (int) var snap_turning_degrees_custom: int = 0

enum play_position_enum { PLAY_POSITION_STANDING, PLAY_POSITION_SEATED }
export (int) var play_position: int = play_position_enum.PLAY_POSITION_STANDING

enum movement_orientation_enum {
	HEAD_ORIENTED_MOVEMENT,
	HAND_ORIENTED_MOVEMENT,
	PLAYSPACE_ORIENTED_MOVEMENT
}
export (int) var movement_orientation: int = movement_orientation_enum.HEAD_ORIENTED_MOVEMENT
export (int) var preferred_hand_oriented_movement_hand: int = hand_enum.LEFT_HAND

enum movement_type_enum { MOVEMENT_TYPE_TELEPORT, MOVEMENT_TYPE_LOCOMOTION }
var movement_type: int = movement_type_enum.MOVEMENT_TYPE_TELEPORT

# 1.0 arm vs 0.0 eye
export (float) var eye_to_arm_ratio = 1.0

# Measured in centimeters
export (float) var custom_player_height: float = 1.8

# Full armspan/wingspan (to fingers).
# Data indicates that the average armspan is that of overall height
export (float) var custom_player_armspan_to_height_ratio: float = 1.0

export (Color) var laser_color = Color(1.0, 0, 0, 0.5)

func set_settings_values_and_save() -> void:
	set_settings_values()
	if save_settings_callback.is_valid():
		save_settings_callback.call_func()
...
func get_settings_values() -> void:
	...
	rotation_sensitivity = get_settings_value("rotation_sensitivity", TYPE_REAL, rotation_sensitivity)
	movement_deadzone = get_settings_value("movement_deadzone", TYPE_REAL, movement_deadzone)
	rotation_deadzone = get_settings_value("rotation_deadzone", TYPE_REAL, rotation_deadzone)
	max_positional_distance_start = get_settings_value("max_positional_distance_start", TYPE_REAL, max_positional_distance_start)
	max_positional_distance_end = get_settings_value("max_positional_distance_end", TYPE_REAL, max_positional_distance_end)
	...
	eye_to_arm_ratio = get_settings_value("eye_to_arm_ratio", TYPE_REAL, eye_to_arm_ratio)
	custom_player_height = get_settings_value("custom_player_height", TYPE_REAL, custom_player_height)
	custom_player_armspan_to_height_ratio = get_settings_value("custom_player_armspan_to_height_ratio", TYPE_REAL, custom_player_armspan_to_height_ratio)
