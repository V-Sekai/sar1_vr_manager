...
export(float) var distance = 1.0 setget set_distance
...
static func xform_plane(p_transform: Transform, p_plane: Plane) -> Plane:
	...
	point = p_transform.xform(point)
	point_dir = p_transform.xform(point_dir)
...
static func get_endpoints_for_camera(p_camera: Camera) -> PoolVector2Array:
	var planes: Array = p_camera.get_frustum()
	
	var camera_gt_inv: Transform = p_camera.global_transform.inverse()
	...
	return PoolVector2Array([Vector2(near_endpoint.x, near_endpoint.y), Vector2(far_endpoint.x, far_endpoint.y)])
...
func update_plane(p_lerp: float) -> void:
	...
	if camera:
		...
		if sizes.size() > 0:
			...
			var lerped_size: Vector2 = far.linear_interpolate(near, p_lerp)
...
func _ready() -> void:
	var unshaded: SpatialMaterial = SpatialMaterial.new()
