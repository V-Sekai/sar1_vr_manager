...
var snapping_points: Array = []

func register_snapping_point(point : Spatial) -> void:
	if(!snapping_points.has(point)):
		# LogManager.printl("registering snap point: " + point.name)
		snapping_points.append(point);

func unregister_snapping_point(point : Spatial) -> void:
	var index : int = snapping_points.find(point)
	if(index >= 0):
		# LogManager.printl("unregistering snap point: " + point.name)
		snapping_points.remove(index);
...
static func calc_redirection_basis(source: Vector3, center: Vector3):
	...
	var up: Vector3 = ARVRServer.get_hmd_transform().basis.y.normalized()
...
static func calc_redirection_dist(point: Vector3, source: Vector3, center: Vector3, redirect_basis:Basis, redirect_direction: Vector2) -> float:
	...
	if(point_vector.angle_to(center_vector) > PI / 4):# if angle is more than 45 degrees away we don't snap
		return INF
	var point_xyz: Vector3 = redirect_basis.xform(point_vector)
